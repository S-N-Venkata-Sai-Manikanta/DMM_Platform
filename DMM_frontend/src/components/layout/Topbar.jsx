  ...
  return (
    <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-slate-200/70 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 px-4 backdrop-blur-xl lg:px-6">
      ...
      <div className="ml-auto flex items-center gap-1.5">
        ...
        <button onClick={() => navigate('/notifications')} className="relative rounded-xl p-2.5 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800">
          ...
          {menuOpen && (
            <>
              ...
              <div className="absolute right-0 z-20 mt-2 w-48 rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-1.5 shadow-card">
                <button onClick={() => { setMenuOpen(false); navigate('/settings'); }} className="w-full rounded-lg px-3 py-2 text-left text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800">
                  Settings
                </button>
